...
const Input = ({ id, name, value, title, errorMessage, type, placeholder, onChange, required }: InputProps) => {
  return (
    <div className='flex flex-col'>
      ...
      {required ?
        <input name={name} className='bg-gray border border-gray2 text-sm rounded-lg focus:ring-blue focus:border-blue block w-full p-2.5'
          value={value} type={type} id={id} placeholder={placeholder} onChange={onChange} required /> :
        <input name={name} className='bg-gray border border-gray2 text-sm rounded-lg focus:ring-blue focus:border-blue block w-full p-2.5'
          value={value} type={type} id={id} placeholder={placeholder} onChange={onChange} />}
      {true && <div className='flex gap-3 mt-2'>
