...
export const phoneSchema = Yup.string().trim().matches(/^\+?\d{1,3}?[-.\s]?\(?\d{1,4}?\)?[-.\s]?\d{1,4}[-.\s]?\d{1,9}$/,"Ingrese un telefono valido").required("Este campo es obligatorio");
...
export const emailSchema =  Yup.string().email("Ingrese un email valido").required("Este campo es obligatorio");
